...
def consultant_answer(user_text):
    ...
    if any(x in q for x in ["что взять", "что посовет", "посовету", "порекоменду", "выбрать"]):
        if any(x in q for x in ["салат", "оливье"]):
            return "🥗 Для салата я бы в первую очередь смотрел на салатное мясо краба — его удобно использовать без красивой порционной подачи. Также можно рассмотреть кальмара или креветку. Если скажете, на сколько человек и какой примерно бюджет, Шкипер подберёт конкретные позиции из каталога."
        if any(x in q for x in ["праздник", "гост", "стол", "день рождения", "новый год"]):
            return "🎉 Для праздничного стола я бы посмотрел на краба, икру и несколько разных морепродуктов, чтобы стол был разнообразным. Скажите, на сколько гостей и какой примерно бюджет — тогда Шкипер посоветует конкретные позиции из каталога."
        if any(x in q for x in ["ужин", "ужинa", "вечер"]):
            return "🍽 Для ужина можно подобрать рыбу, гребешок, кальмара или другие морепродукты. Скажите, будете готовить горячее или хотите просто поставить закуску на стол — Шкипер посоветует точнее."
