...
MAX_LINK = "https://max.ru"  # Ссылка на мессенджер Max
...
# ==================== ПРОВЕРКА ПОДПИСКИ (ИСПРАВЛЕННАЯ) ====================
def subscription_required(channel=CHANNEL_ID):
    def decorator(handler):
        async def wrapper(*args, **kwargs):
            # Находим msg в args (первый аргумент — обычно Message или Update)
            msg = None
            for arg in args:
                if isinstance(arg, types.Message):
                    ...
                    break
            if not msg:
                # Если не нашли Message, просто вызываем handler
