...
def consultant_answer(user_text):
    ...
    if any(x in q for x in ["что взять", "что посовет", "посовету", "порекоменду", "выбрать"]):
        ...
        if any(x in q for x in ["праздник", "гост", "стол", "день рождения", "новый год"]):
            items = catalog_recommendations(["краб", "икра", "гребеш", "кревет"], 6)
            return attractive_catalog_presentation(
                items,
    ...
    # --- Общий запрос без конкретного продукта ---
    if any(x in q for x in ["посоветуй что-нибудь", "что-нибудь вкусн", "что взять", "посоветуй", "порекоменду", "что выбрать"]):
        if not any(x in q for x in ["салат", "праздник", "гост", "стол", "ужин", "краб", "икр", "рыб", "кальмар", "гребеш", "кревет"]):
            items = catalog_recommendations(["икра", "краб", "рыб", "гребеш", "кревет"], 6)
            if items:
                return attractive_catalog_presentation(
                    items,
                    "⚓ Давайте посмотрим, чем можно себя порадовать. Шкипер выбрал несколько разных вариантов из актуального каталога — не только привычные позиции:"
                )
    ...
    # --- Икра ---
    if any(x in q for x in ["икра", "кета", "горбуш", "нерк", "щуч", "щук", "ёж", "еж", "уни"]):
        roe_items = catalog_recommendations(["икра", "щуч", "щуки", "еж", "ёж", "уни"], 6)

        if any(x in q for x in ["ёж", "еж", "уни"]):
            answer = (
                "🌊 <b>Икра морского ежа — совсем другая история.</b>\n\n"
                "Это не просто «ещё одна икра»: у неё нежная, кремовая текстура и ярко выраженный морской вкус. "
                "Такой продукт обычно берут, когда хочется попробовать что-то необычное или сделать действительно эффектную подачу."
            )
            if roe_items:
                answer += "\n\n⚓ <b>Смотрю, что есть у нас в каталоге:</b>\n" + "\n".join("• " + x for x in roe_items)
            return answer

        if any(x in q for x in ["щуч", "щук"]):
            answer = (
                "🥚 <b>Икра щуки</b> — хороший вариант, если хочется икру с более мелким зерном и характерной текстурой. "
                "Её удобно подавать самостоятельно, на тостах или в холодных закусках."
            )
            if roe_items:
                answer += "\n\n⚓ <b>Вот что нашёл в актуальном каталоге:</b>\n" + "\n".join("• " + x for x in roe_items)
            return answer

        if any(x in q for x in ["разниц", "отлич", "какую", "какая", "выбрат", "лучше"]):
            answer = "🥚 <b>У нас не только лососёвая икра.</b> Шкипер может посмотреть разные варианты и подобрать по вкусу и случаю.\n\n"
            if roe_items:
                answer += "<b>В актуальном каталоге:</b>\n" + "\n".join("• " + x for x in roe_items) + "\n\n"
            answer += knowledge_text("types") + "\n\n" + knowledge_text("how_to_choose")
            return answer

        answer = "🥚 <b>Про икру</b>\n\n" + knowledge_text("types")
        if roe_items:
            answer += "\n\n⚓ <b>И вот что сейчас есть в каталоге:</b>\n" + "\n".join("• " + x for x in roe_items)
        return answer
