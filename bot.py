...
main_menu.add(KeyboardButton("🎁 Акции"), KeyboardButton("🛍 Магазин"))
...
async def handle_shop_cmd(message: types.Message):
    await bot.send_invoice(
        chat_id=message.chat.id,
        title="Краб Камчатский",
        description="Свежий, крупный, вылов 2025 года. 1 кг — 3200₽",
        payload="krab_001",
        provider_token="381764678:TEST:29525",  # тестовый токен Telegram
        currency="RUB",
        prices=[types.LabeledPrice(label="Краб Камчатский", amount=320000)],
        photo_url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQNM020wVzpvuLMtcFmvVDZ347_csYIIXnjTA&s",
        photo_height=512,
        photo_width=512,
        photo_size=512,
        start_parameter="krabkamchat"
    )

@dp.message_handler(lambda message: message.text == "🛍 Магазин")
async def shop_button_handler(message: types.Message):
    await handle_shop_cmd(message)
